...
namespace ScratchCardAsset.Core
{
    ...
    public class ScratchCardRenderer
    {
        ...
        /// <summary>
        /// Draws quad into RenderTexture with custom Brush
        /// </summary>
        public void ScratchHoleWithBrush(Vector2 position, float pressure = 1f, Material brushMaterial = null)
        {
            if (brushMaterial == null)
            {
                brushMaterial = scratchCard.BrushMaterial;
            }

            var positionRect = new Rect(
                (position.x - 0.5f * brushMaterial.mainTexture.width * scratchCard.BrushSize * pressure) / scratchCard.ScratchData.TextureSize.x,
                (position.y - 0.5f * brushMaterial.mainTexture.height * scratchCard.BrushSize * pressure) / scratchCard.ScratchData.TextureSize.y,
                brushMaterial.mainTexture.width * scratchCard.BrushSize * pressure / scratchCard.ScratchData.TextureSize.x,
                brushMaterial.mainTexture.height * scratchCard.BrushSize * pressure / scratchCard.ScratchData.TextureSize.y);
            ...
            if (IsInBounds(positionRect))
            {
                ...
                commandBuffer.Clear();
                commandBuffer.SetRenderTarget(scratchCard.RenderTarget);
                commandBuffer.DrawMesh(meshHole, Matrix4x4.identity, brushMaterial);
                Graphics.ExecuteCommandBuffer(commandBuffer);
                IsScratched = true;
            }
        }
