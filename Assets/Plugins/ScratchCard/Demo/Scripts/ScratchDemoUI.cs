...
namespace ScratchCardAsset.Demo
{
    public class ScratchDemoUI : MonoBehaviour
    {
        ...
        {
            swordRenderer.transform.position += Vector3.right * Time.deltaTime;
            var swordRendererPosToCam = cardManager.MainCamera.WorldToScreenPoint(swordRenderer.transform.position);
            var normalizedPosition = cardManager.Card.ScratchData.GetScratchPosition(swordRendererPosToCam);
            cardManager.Card.ScratchLine(normalizedPosition - Vector2.right * 80f, normalizedPosition + Vector2.right * 80f);
        }

        private void ScratchCircle()
        {
            circleRenderer.transform.position += Vector3.right * Time.deltaTime;
            var circleRendererPosToCam = cardManager.MainCamera.WorldToScreenPoint(circleRenderer.transform.position);
            var normalizedPosition = cardManager.Card.ScratchData.GetScratchPosition(circleRendererPosToCam);
            cardManager.Card.ScratchHole(normalizedPosition);
        }
