...
{
    /// <summary>
    /// Holds the scene references: ground, mask, wall colliders and fence prefabs.
    /// Does no math — it just applies whatever the Arena tells it to.
    /// </summary>
    public sealed class ArenaView : MonoBehaviour
    {
        [Header("Ground (SpriteRenderer — Simple)")]
        [SerializeField] private SpriteRenderer _ground;
        [SerializeField] private SpriteRenderer _mask;

        [Header("Wall colliders (no Rigidbody2D)")]
        ...
        [Header("Fence prefabs (SpriteRenderer — Simple)")]
        ...
        /// <summary>
        /// Simple draw mode → size comes from SCALE, not renderer.size.
        /// scale = desiredSize / sprite.bounds.size
        /// </summary>
        public void SetArenaSize(Vector2 size, float wallThickness, float offset)
        {
            ...
            // Colliders sit just OUTSIDE the play area
        ...
        private static void ApplyScale(SpriteRenderer renderer, Vector2 size, float offset)
        {
            ...
            Vector2 unit = renderer.sprite.bounds.size;      // world size at scale 1
